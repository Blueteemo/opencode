...
function applyTestToken(text: string) {
  return text.replace(/\{env:TEST_TOKEN\}/g, "test-token")
}
  ...
  Layer.mock(AuthWellKnown.Service)({
    configs: () => {
      ...
      if (input.remoteConfig) {
        const remoteURL = applyTestToken(input.remoteConfig.url)
        input.seen.remote = remoteURL
        const authorization = input.remoteConfig.headers?.Authorization
        if (authorization) input.seen.authorization = applyTestToken(authorization)
        const payload = input.remote
        const content = payload && typeof payload === "object" && "config" in payload ? payload.config : payload
        documents.push({
          url: remoteURL,
          source: remoteURL,
...
const wellKnown = (input: {
  ...
  const value = metadata as { config?: unknown; remote_config?: { url: string; headers?: Record<string, string> } }
  return {
    seen,
    it: configIt({
      auth: wellKnownAuth({
        ...
        config: value.config,
        remoteConfig: value.remote_config,
        remote: input.remote,
        seen,
      }),
    }),
